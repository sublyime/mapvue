...
class FeatureModel {
    static async create(featureData, ownerId) {
        const query = `
    ...
    static async update(id, featureData, userId) {
    ...
    static async getByLayer(layerId, userId) {
        const { features } = await this.list({ layer_id: layerId }, userId);
        return features;
    }
    static async getInBounds(bounds, userId) {
    ...
    static async search(searchTerm, layerId, userId) {
        const { features } = await this.list({ search: searchTerm, layer_id: layerId }, userId);
        return features;
    }
    static async bulkCreate(featuresData, ownerId) {
        ...
        for (const featureData of featuresData) {
            ...
            values.push(featureData.layer_id, ownerId, featureData.name || null, featureData.description || null, JSON.stringify(featureData.geometry), featureData.properties || {}, featureData.style || {}, featureData.is_visible !== undefined ? featureData.is_visible : true);
            paramCount += 8;
        }
        const query = `
      INSERT INTO features (
    ...
    static async getExtent(layerId) {
        const query = `
