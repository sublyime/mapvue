...
export class LayerModel {
  ...
  static async findById(id: string, userId?: string): Promise<Layer | null> {
  ...
  static async update(id: string, layerData: UpdateLayerRequest, userId: string): Promise<Layer | null> {
  ...
  static async getByProject(projectId: string, userId?: string): Promise<Layer[]> {
    const { layers } = await this.list({ project_id: projectId }, userId);
    return layers;
  }

  static async getStats(id: string): Promise<LayerStats | null> {
    const query = `
  ...
  static async updateOrder(layerId: string, newOrder: number, userId: string): Promise<boolean> {
